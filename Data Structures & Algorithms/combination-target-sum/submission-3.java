class Solution {
    ...
    private void backtracking(int[] nums, int remain, int start, List<List<Integer>> result, List<Integer> list){
        // Total sum of list = target
        if(remain == 0){
            result.add(new ArrayList<>(list));
            return;
        }

        // Total sum > target (backtrack)
        if(remain < 0){
            return;
        }

        // Possible step at current index and the other in nums
        for(int i = start; i < nums.length; i++){
            //When last index is smallest value but its total list > target 
